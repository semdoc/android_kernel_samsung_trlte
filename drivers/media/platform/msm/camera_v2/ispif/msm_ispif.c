...
static void msm_ispif_io_dump_start_reg(struct ispif_device *ispif)
{
	if (!ispif->enb_dump_reg)
		return;
	msm_camera_io_dump_3(ispif->base, 0x500);
}
	...
	struct msm_ispif_param_data *params)
{
	...
	for (i = 0; i < params->num; i++) {
		...
		CDBG("%s intf_cmd 0x%x cmd_bits 0x%x\n", __func__,
	...
	struct msm_ispif_param_data *params)
{
	...
	if (ispif->ispif_state != ISPIF_POWER_UP) {
		rc = -EPERM;
		pr_err("%s:error!  ispif invalid state %d rc = %d\n", __func__,
			ispif->ispif_state, rc );
		return rc;
	}
...
*/
	...
	CDBG("%s: <overflow_dbg> ISPIF disable clk after reset hw is done", __func__);
	rc = msm_cam_clk_enable(&ispif->pdev->dev,
		ispif_8974_reset_clk_info, reset_clk,
		ARRAY_SIZE(ispif_8974_reset_clk_info), 0);
	if (rc < 0) {
...
*/
		...
		msm_ispif_intf_cmd(ispif, ISPIF_INTF_CMD_ENABLE_FRAME_BOUNDARY, params);
		CDBG("%s overflow_dbg intftype %x, vfe_intf %d\n", __func__,
			intftype, vfe_intf);
	}
